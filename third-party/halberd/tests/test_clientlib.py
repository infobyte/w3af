...
class TestHTTPClient(unittest.TestCase):
    ...
    def testGetHostAndPort(self):
        self.assertEqual(self.client._getHostAndPort('localhost:8080'),
                             ('localhost', 8080))

        self.assertEqual(self.client._getHostAndPort('localhost'),
    ...
    def testFillTemplate(self):
        ...
        self.assertTrue(req.splitlines()[:2] == \
        ...
        self.assertTrue(req.splitlines()[0] == 'GET / HTTP/1.1')

    def testAntiCache(self):
        req = self.client._fillTemplate('localhost', 80, '/index.html')
        self.assertTrue(req.splitlines()[2:4] == \
...
class TestHTTPSClient(unittest.TestCase):
    ...
    def testGetHostAndPort(self):
        self.assertEqual(self.client._getHostAndPort('secure'),
                             ('secure', self.client.default_port))

        self.assertEqual(self.client._getHostAndPort('secure:777'),
                             ('secure', 777))
    ...
    def testInvalidConnect(self):
        self.assertRaises(clientlib.HTTPSError,
    ...
    def testSendRequestToRemote(self):
        ...
        timestamp, headers = self.client._getReply()
        self.assertTrue(headers != None and headers.startswith('HTTP/'))
